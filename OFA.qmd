...
# For nanosecond timestamps like "2017-10-11 03:10:34.852660"
quote_data_all <- quote_data_all |>
  mutate(
...
# Define k values (sampling intervals)
...
for (k in k_values) {
  # Subsample the mid-price: take every k-th observation
  subsampled <- quote_data_day$mid_price[seq(1, nrow(quote_data_day), by = k)]
  
  # calculate differenced price series ΔPk
  dpk <- diff(subsampled)
  
  # Perform ADF and KPSS tests
  adf_result <- adf.test(dpk, alternative = "stationary")
  kpss_result <- kpss.test(dpk, null = "Level")

  # Store p-values
  results <- rbind(results, data.frame(
    k = k,
    `ADF p-value` = adf_result$p.value,
    `KPSS p-value` = kpss_result$p.value
  ))
}

# Output using kable
kable(results, digits = 4, caption = "ADF and KPSS Test Results for ΔPk (Various Sampling Intervals)")
...
# Example: differenced series
dpk <- diff(quote_data_day$mid_price)

# Run ADF test with a drift term (can also use "none" or "trend")
adf_result <- ur.df(dpk, type = "drift", selectlags = "AIC")

# Extract test statistic
adf_stat <- adf_result@teststat[1]

# Extract 1% critical value
adf_crit_1pct <- adf_result@cval[1, "1pct"]
...
# Step 1: Count quote updates in 10-second intervals
...
# Step 2: Compute ACF for lags 0–10
acf_result <- acf(arrival_10s$updates_per_10s, lag.max = 9, plot = FALSE)
...
## Fig 4:  XBTUSD 1-s order flow imbalance versus contemporaneous price change
```{r}
# 1. Aggregate OFI to 1-second intervals
ofi_1s <- quote_data_all %>%
  mutate(interval_time = floor_date(timestamp, unit = "1 second")) %>%
  group_by(interval_time) %>%
  summarise(OFI = sum(OFI, na.rm = TRUE), .groups = "drop")

# 2. Aggregate mid-price and compute tick changes
tick_size <- 0.1  # smallest observed price increment (5 cents)

midprice_1s <- quote_data_all %>%
  mutate(interval_time = floor_date(timestamp, unit = "1 second")) %>%
  group_by(interval_time) %>%
  summarise(mid_price = mean(mid_price, na.rm = TRUE), .groups = "drop") %>%
  mutate(price_change_ticks = (mid_price - lag(mid_price)) / tick_size)

# 3. Merge OFI and mid-price change data
ofi_vs_price <- inner_join(ofi_1s, midprice_1s, by = "interval_time") %>%
  drop_na()

# 4. Scatter plot
ggplot(ofi_vs_price, aes(x = OFI, y = price_change_ticks)) +
...
```
##  Fig. 5  XBTUSD 1-min order flow imbalance versus contemporaneous price change
```{r}
#| fig-height: 5
#| fig-width: 7
library(dplyr)
...
# # 1. Aggregate OFI to 1-minute intervals
# ofi_1m <- quote_data_all %>%
#   mutate(interval_time = floor_date(timestamp, unit = "1 minute")) %>%
#   group_by(interval_time) %>%
#   summarise(OFI = sum(OFI, na.rm = TRUE), .groups = "drop")
# 
# # 2. Aggregate mid-price and compute tick changes at 1-minute intervals
# tick_size <- 0.1
# 
# midprice_1m <- quote_data_all %>%
#   mutate(interval_time = floor_date(timestamp, unit = "1 minute")) %>%
#   group_by(interval_time) %>%
#   summarise(mid_price = mean(mid_price, na.rm = TRUE), .groups = "drop") %>%
#   mutate(price_change_ticks = (mid_price - lag(mid_price)) / tick_size)
# 
# # 3. Merge OFI and mid-price change data
# ofi_vs_price_1m <- inner_join(ofi_1m, midprice_1m, by = "interval_time") %>%
#   drop_na()

# 4. Scatter plot for 1-minute OFI
ggplot(ofi_vs_price_1m, aes(x = OFI, y = price_change_ticks)) +
  geom_point(alpha = 0.5, size = 0.8, color = "steelblue") +
  labs(
...
### Figure 7: XBTUSD 1-min trade flow imbalance versus contemporaneous price change. R2 = 58.6%
...
ggplot(one_min_results, aes(x = TFI, y = mid_price_change)) +
  ...
  scale_x_continuous(
  labels = function(x) {
    paste0(x/1e7)  # Express everything in terms of 1e7
  },
  breaks = seq(-0.75e7, 0.75e7, by = 0.25e7),
...
ggsave("XBTUSD_1min_TFI_Mid_Price_Chnage.png", width = 6, height = 5, dpi = 300, bg = "white")
```
