...
# Step 2: Compute ACF of 10-second OFI (limit to 10 lags)
...
# Step 4: Compute confidence bounds
N_ofi <- length(ofi_10s$OFI)
conf_interval_ofi <- 1.96 / sqrt(N_ofi)

# Step 5: Plot OFI ACF as bars with CI lines (same format as previous)
ggplot(acf_ofi_df, aes(x = Lag, y = ACF)) +
  ...
  labs(title = "XBTUSD 10-Second Order Flow Imbalance ACF",
       x = "Lag",
       y = "Autocorrelation") +
  theme_minimal()
