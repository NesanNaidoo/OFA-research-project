...
## Loading in data
```{r}
quote_data_all<-read.csv("_raw_data/XBTUSD_quotes_20171001_20171023.csv",header = TRUE)
trade_data_all<-read.csv("_raw_data/XBTUSD_trades_20171001_20171023.csv",header = TRUE)
```
...
p1 <- ggplot(quote_data_day, aes(x = timestamp, y = mid_price, color = "midprice")) +
  ...
  scale_x_datetime(
    breaks = breaks_seq,
    labels = NULL  # Hide x labels
  ) +
  labs(x = NULL, y = "Mid-Price (USD)", color = NULL) +
  theme(
...
p2 <- ggplot(quote_data_day, aes(x = timestamp, y = mid_price_change, color = "dmidprice")) +
  ...
  labs(x = NULL, y = "Mid-Price change (USD)", color = NULL) +
  theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    panel.grid.major = element_line(color = "gray80"),
...
p3 <- ggplot(quote_data_day, aes(x = timestamp, y = rolling_volatility, color = "vol")) +
  ...
  scale_x_datetime(
    breaks = breaks_seq,
    labels = labels_seq  # Show formatted labels here only
  ) +
  labs(x = "", y = "Std. dev. of Mid-Price change", color = NULL) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    panel.grid.major = element_line(color = "gray80"),
    panel.grid.minor = element_blank(),
...
perform_stationarity_tests <- function(series) {
...
price_stationarity_results <- data.frame()

for (int in intervals) {
  # Aggregate mid_price by interval
  agg <- quote_data_all %>%
    mutate(interval_time = floor_date(timestamp, unit = int)) %>%
    group_by(interval_time) %>%
    summarise(mid_price = mean(mid_price, na.rm = TRUE), .groups = "drop")
  
  # Calculate differenced price series ΔPₖ
  dpk <- diff(agg$mid_price)
  
  # Run tests
  test_results <- perform_stationarity_tests(dpk)
  test_results$Interval <- int
  
  price_stationarity_results <- bind_rows(price_stationarity_results, test_results)
}

price_stationarity_results <- price_stationarity_results %>%
  select(Interval, everything())

kable(price_stationarity_results, digits = 4,
      caption = "ADF and KPSS Test Results for ΔPₖ at Different Sampling Periods")
...
perform_stationarity_urca <- function(series) {
  ...
  data.frame(
    ADF_Statistic = adf@teststat[1], # τ statistic
...
kable(price_stationarity_urca, digits = 6,
      caption = "ADF and KPSS Test Results for ΔPₖ (URCA package)")
