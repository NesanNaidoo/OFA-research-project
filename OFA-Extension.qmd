...
## Loading in data
```{r}
quote_data_all<-read.csv("XBTUSD_quotes_20241001_20241031.csv",header = TRUE)
trade_data_all<-read.csv("XBTUSD_trades_20241001_20241031.csv",header = TRUE)
...
# 15th October 2024
# Prepare mid-price data for 15 Oct 2024
quote_data_day <- quote_data_all |>
  mutate(timestamp = ymd_hms(timestamp, tz = "UTC")) |>
  filter(as_date(timestamp) == as_date("2024-10-15")) |>
  mutate(mid_price = (bidPrice + askPrice) / 2)

start_time <- as.POSIXct("2024-10-15 03:00:00", tz = "UTC")
end_time   <- as.POSIXct("2024-10-15 21:00:00", tz = "UTC")
breaks_seq <- seq(from = start_time, to = end_time, by = "3 hours")

# XBTUSD Mid-Price Series (15 Oct 2024)
...
# Plot # Tick-to-Tick Mid-Price Change (15 Oct 2024)
...
p1 <- ggplot(quote_data_day, aes(x = timestamp, y = mid_price, color = "midprice")) +
  ...
  legend.title = element_text(size = 8)  ,   # legend title (if present)
...
p2 <- ggplot(quote_data_day, aes(x = timestamp, y = mid_price_change, color = "dmidprice")) +
  ...
  legend.title = element_text(size = 8) ,
   legend.key.size = unit(0.3, "lines"),
  legend.spacing.y = unit(0.2, "lines"),
  legend.margin = margin(3, 3, 3, 3)# legend title (if present)
  )
...
summary(quote_data_day$mid_price_change)
...
```{r}
#To examine stationarity of the differenced price series, augmented Dickey–Fuller (ADF) and KPSS tests are conducted or all variants of k in ΔPk variable. The tests confirm that price series are stationary  for every sampling period k at 1% significance level.
library(dplyr)
library(lubridate)
library(urca)
library(knitr)
...
kable(price_stationarity_urca, digits = 6,
      caption = "ADF and KPSS (1% critical values) for ΔPₖ (URCA)")
...
tick_size <- 0.1  # BitMEX XBTUSD tick size

quote_data_all <- quote_data_all %>%
  mutate(spread_ticks = (askPrice - bidPrice) / tick_size)

# Summary statistics
spread_summary <- quote_data_all %>%
  summarise(
    avg_spread = mean(spread_ticks, na.rm = TRUE),
    sd_spread = sd(spread_ticks, na.rm = TRUE),
    min_spread = min(spread_ticks, na.rm = TRUE),
    max_spread = max(spread_ticks, na.rm = TRUE)
  )

kable(spread_summary, digits = 2, caption = "Bid–Ask Spread Summary (in ticks)")
...
```{r}
# --- Aggregate ratio ---
n_quotes <- nrow(quote_data_all)
n_trades <- nrow(trade_data_all)
quote_trade_ratio <- n_quotes / n_trades

cat("Quote-to-trade ratio:", round(quote_trade_ratio, 2), "\n")

```



## Table 1: XBTUSD 1-Minute Arrival Rate Statistics
```{r}

arrival_rates <- trade_data_all |>
  mutate(
    timestamp = as.POSIXct(timestamp, format = "%Y-%m-%d %H:%M:%OS", tz = "UTC"),
    second_bucket = floor_date(timestamp, unit = "1 min")
  ) |>
  count(second_bucket, name = "updates_per_minute")

summary_table1 <- arrival_rates |>
  summarise(
    Mean = mean(updates_per_minute),
    SD = sd(updates_per_minute),
    Min = min(updates_per_minute),
    `25%` = quantile(updates_per_minute, 0.25),
    `50%` = median(updates_per_minute),
    `75%` = quantile(updates_per_minute, 0.75),
    Max = max(updates_per_minute)
  )
...
```
## XBTUSD 1-Minute Arrival Rate Statistics - Volume per minute
```{r}
volume_summary_crypto <- trade_data_all |>
  mutate(
    minute_bucket = floor_date(timestamp, unit = "1 min")
  ) |>
  group_by(minute_bucket) |>
  summarise(total_volume = sum(Volume)) |>
  ungroup() |>
  summarise(
    Mean = mean(total_volume),
    SD = sd(total_volume),
    Min = min(total_volume),
    `25%` = quantile(total_volume, 0.25),
    `50%` = median(total_volume),
    `75%` = quantile(total_volume, 0.75),
    Max = max(total_volume)
  )
print(volume_summary_crypto)
```


## S&P 500 ETF index arrival statistics - volume per minute
```{r}
# Read the Excel file
spy_data <- read_excel("S&P500 ETF Trust_October 2024.xlsx", sheet = "Sheet 1")

# Extract the historical price data section (starting from "Local Date" header)
# Find the starting row of the historical data
start_row <- which(spy_data[[1]] == "Local Date") + 1

# Read the data properly (adjust range as needed based on your file structure)
spy_history <- read_excel("S&P500 ETF Trust_October 2024.xlsx", sheet = "Sheet 1", 
                         skip = start_row - 1, col_names = TRUE)

# Clean up the column names and select relevant columns
colnames(spy_history) <- c("Local Date", "Local Time", "Close", "Open", "Volume", "V6", "V7", "V8", "V9", "V10", "V11", "V12", "V13")

# Filter out empty rows and create proper timestamp
spy_trade_data <- spy_history |>
  filter(!is.na(`Local Date`) & !is.na(Volume) & Volume > 0) |>
  mutate(
    timestamp = as.POSIXct(paste(`Local Date`, `Local Time`), 
...
# Create summary table (same format as your crypto code)
summary_table_spy <- spy_trade_data |>
  summarise(
    Mean = mean(Volume, na.rm = TRUE),
    SD = sd(Volume, na.rm = TRUE),
    Min = min(Volume, na.rm = TRUE),
    `25%` = quantile(Volume, 0.25, na.rm = TRUE),
    `50%` = median(Volume, na.rm = TRUE),
    `75%` = quantile(Volume, 0.75, na.rm = TRUE),
    Max = max(Volume, na.rm = TRUE)
  )

print(summary_table_spy)
```
...
## Time series Update Arrival Volumes
```{r}
...
# Aggregate quote updates into 10-second intervals
arrival_10s <- quote_data_all |>
  mutate(ten_sec_bucket = floor_date(timestamp, unit = "10 seconds")) |>
  count(ten_sec_bucket, name = "updates_per_10s")

# Time series plot of update arrival volumes
ggplot(arrival_10s, aes(x = ten_sec_bucket, y = updates_per_10s)) +
  geom_line(color = "steelblue") +
  labs(
    title = "Update Arrival Volumes (10-Second Intervals)",
    x = "Time",
    y = "Number of Quote Updates"
  ) +
  scale_x_datetime(
    date_breaks = "5 days",
...
## ADF and PP tests for Update Arrival Volume
```{r}
# ==========================================
# Stationarity Tests: ADF and PP (urca)
# ==========================================
...
# Extract the series of update counts
update_counts <- arrival_10s$updates_per_10s

# 1. Augmented Dickey–Fuller (ADF) test
# The null hypothesis (H0): series has a unit root (non-stationary)
adf_test <- ur.df(update_counts, type = "drift", selectlags = "AIC")
summary(adf_test)

# 2. Phillips-Perron (PP) test
# The null hypothesis (H0): series has a unit root (non-stationary)
pp_test <- ur.pp(update_counts, type = "Z-tau", model = "constant", lags = "short")
summary(pp_test)

# Optional: Create a concise summary table for reporting
adf_stat <- adf_test@teststat[1]    # ADF t-statistic
adf_crit <- adf_test@cval[1, ]      # Critical values
pp_stat <- pp_test@teststat[1]      # PP t-statistic
pp_crit <- pp_test@cval[1, ]        # Critical values

stationarity_results <- data.frame(
  Test = c("ADF (drift)", "PP (Z-tau)"),
  Statistic = c(round(adf_stat, 3), round(pp_stat, 3)),
  `5% Critical Value` = c(adf_crit["5pct"], pp_crit["5pct"]),
  `Decision (5%)` = c(
    ifelse(adf_stat < adf_crit["5pct"], "Reject H0 (Stationary)", "Fail to reject H0 (Non-stationary)"),
    ifelse(pp_stat < pp_crit["5pct"], "Reject H0 (Stationary)", "Fail to reject H0 (Non-stationary)")
  )
)
...
# 1. Serial Correlation Check (Ljung-Box Test)
residuals_adf <- adf_test@res  # Access residuals via @res slot
Box.test(residuals_adf, lag = 10, type = "Ljung-Box")

# 2. Heteroskedasticity Check (Breusch-Pagan Test)
model <- lm(update_counts ~ lag(update_counts, 1), data = data.frame(update_counts = as.zoo(update_counts)))
bptest(model)
```



## ADF and KPSS all intervals OFI (Contradictory Results)
```{r}

# Define intervals
intervals <- c("1 sec", "10 sec", "1 min", "5 min", "10 min", "1 hour")

# Custom test function using urca with drift for ADF
perform_ofi_tfi_tests <- function(series) {
  series <- as.numeric(series)
  series <- series[is.finite(series)]
  ...
  kpss_cv1 <- if (!is.null(names(kpss_cv)) && "1pct" %in% names(kpss_cv)) {
    ...
    as.numeric(kpss_cv[length(kpss_cv)])  # fallback
  }
...
####################################################
# OFI
ofi_stationarity_urca <- data.frame()

for (int in intervals) {
  ofi_agg <- aggregate_ofi(quote_data_all, interval = int)
  
  test_results <- perform_ofi_tfi_tests(ofi_agg$OFI)
  test_results$Interval <- int
  
  ofi_stationarity_urca <- bind_rows(ofi_stationarity_urca, test_results)
}

ofi_stationarity_urca <- ofi_stationarity_urca %>%
  select(Interval, everything())

kable(ofi_stationarity_urca, digits = 5,
      caption = "ADF and KPSS Tests for OFI (URCA, 1% CV)")


```


## ADF and PP affected intervals OFI (new)
```{r}
# ==========================================
# Stationarity Tests: ADF and PP for OFI (Specific Intervals)
# ==========================================
...
# Define specific intervals of interest (contradictory: 1 sec, 10 sec, 1 min)
intervals <- c("1 sec", "10 sec", "1 min")

# Custom test function using urca with drift for ADF and PP
perform_ofi_tests <- function(series) {
  series <- as.numeric(series)
  series <- series[is.finite(series)]
  ...
  if (length(series) < 10) {
    return(data.frame(
      ADF_Statistic = NA_real_, ADF_CV_1pct = NA_real_,
      PP_Statistic = NA_real_, PP_CV_1pct = NA_real_
    ))
  }

  ## ADF test with none
  adf <- ur.df(series, type = "none", selectlags = "AIC")
  adf_stat <- as.numeric(adf@teststat[1])
  adf_cv1 <- as.numeric(adf@cval[1, "1pct"])

  ## PP test with Z-tau and constant
  pp <- ur.pp(series, type = "Z-alpha", model = "none", lags = "short")
  pp_stat <- as.numeric(pp@teststat[1])
  pp_cv1 <- as.numeric(pp@cval[1, "1pct"])
  summary(pp)
...
perform_residual_diagnostics <- function(series, interval) {
  series <- as.numeric(series)
  series <- series[is.finite(series)]
  
  if (length(series) < 10) return(data.frame(Serial_Corr_p = NA, Hetero_p = NA))
  ...
  serial_test <- Box.test(residuals, lag = 10, type = "Ljung-Box")
  serial_p <- serial_test$p.value
  
  # Heteroskedasticity (Breusch-Pagan Test) - Proxy AR(1) model
  model <- lm(series ~ lag(series, 1), data = data.frame(series = as.zoo(series)))
  hetero_test <- bptest(model)
  hetero_p <- hetero_test$p.value
  
  data.frame(Serial_Corr_p = serial_p, Hetero_p = hetero_p, Interval = interval)
}

# OFI Stationarity Analysis
ofi_stationarity_urca <- data.frame()
...
for (int in intervals) {
  ...
  # Stationarity tests
  test_results <- perform_ofi_tests(ofi_agg$OFI)
  test_results$Interval <- int
  
  ofi_stationarity_urca <- bind_rows(ofi_stationarity_urca, test_results)
  
  # Residual diagnostics
  diag_results <- perform_residual_diagnostics(ofi_agg$OFI, int)
  ofi_stationarity_urca <- bind_rows(ofi_stationarity_urca, diag_results)
}

# Reorder and select columns
ofi_stationarity_urca <- ofi_stationarity_urca %>%
  select(Interval, ADF_Statistic, ADF_CV_1pct, PP_Statistic, PP_CV_1pct, Serial_Corr_p, Hetero_p)

# Create table
kable(ofi_stationarity_urca, digits = 5,
      caption = "ADF and PP Tests with Residual Diagnostics for OFI (URCA, 1% CV, Specific Intervals)")
```


# 5.3 Trade flow imbalance (TFI)(Wrong package)
...
## ADF and KPSS tests-all intervals
```{r}
##############################
# TFI
tfi_stationarity_urca <- data.frame()

for (int in intervals) {
  tfi_agg <- aggregate_tfi(trade_data_all, quote_data_all, interval = int)
  
  test_results <- perform_ofi_tfi_tests(tfi_agg$TFI)
  test_results$Interval <- int
  
  tfi_stationarity_urca <- bind_rows(tfi_stationarity_urca, test_results)
}

tfi_stationarity_urca <- tfi_stationarity_urca %>%
  select(Interval, everything())

kable(tfi_stationarity_urca, digits = 5,
      caption = "ADF and KPSS Tests for TFI (URCA, 1% CV)")
```
...
# Extra
##  XBTUSD 1-hour OFI vs Mid-price change
```{r}
#| fig-height: 5
#| fig-width: 7
...
# Scatter plot for 1-hour OFI
ggplot(ofi_vs_price_1m, aes(x = OFI, y = mid_price_change)) +
  # Points first
  geom_point(alpha = 1, size = 1, color = "steelblue") +
...
##  XBTUSD 1-sec TFI vs mid price change.
```{r}
#| fig-width: 6
#| fig-height: 5
# Filter for 1-minute interval
one_min_results <- aggregate_tfi(trade_data_all, quote_data_all, interval = "1 sec") 
...
ggplot(one_min_results, aes(x = TFI, y = mid_price_change)) +
  geom_point(alpha = 0.70, size = 1, shape = 21, color = "steelblue", fill = "steelblue") +
  geom_vline(xintercept = x_breaks, color = "grey80", linewidth = 0.1) +
  geom_hline(yintercept = y_breaks, color = "grey80", linewidth = 0.1) +
  scale_y_continuous(
    breaks = y_breaks,
...
##  XBTUSD 1-hour TFI vs mid price change.
```{r}
#| fig-width: 6
#| fig-height: 5
# Filter for 1-minute interval
one_min_results <- aggregate_tfi(trade_data_all, quote_data_all, interval = "1 hour") 
