---
title: "Order Flow Analysis-Extension period (October 2024)"
...
## Loading in data
```{r}
quote_data<-read.csv("XBTUSD_quotes_20241001_20241031.csv",header = TRUE)
trade_data<-read.csv("XBTUSD_trades_20241001_20241031.csv",header = TRUE)
```

## Cleaning and formatting data 
```{r}
## Cleaning data so it matches data description of Silyantyev(2019)
quote_data<-quote_data |> select(timestamp,bidPrice,bidSize,askPrice,askSize) 
quote_data<-quote_data |> rename(bidVolume=bidSize,askVolume=askSize)
...
trade_data<-trade_data |> select(timestamp,price,size,side)
trade_data<-trade_data |> rename(Volume=size)
...
# to get mid-price change and OFI per interval
...
```{r}
# function to run lm an extract relevant stats
...
```{r}
# --- Aggregate ratio ---
n_quotes <- nrow(quote_data)
n_trades <- nrow(trade_data)
...
for (int in intervals) {
  ofi_agg <- aggregate_ofi(quote_data, interval = int)
...
# Plot
ggplot(ofi.1s, aes(x = OFI, y = mid_price_change)) +
...
ggplot(ofi.1m, aes(x = OFI, y = mid_price_change)) +
...
# Trade flow imbalance (TFI)(Wrong package)
...
for (int in intervals) {
  ...
  tfi_data <- aggregate_tfi(trade_data, quote_data, interval = int)
...
for (int in intervals) {
  tfi_agg <- aggregate_tfi(trade_data, quote_data, interval = int)
...
tfi_10s <- aggregate_tfi(trade_data, quote_data)
...
one_min_results <- aggregate_tfi(trade_data, quote_data, interval = "1 min") 
...
for (intv in intervals) {
  ofi_data <- aggregate_ofi(quote_data, interval = intv)
  tfi_data <- aggregate_tfi(trade_data, quote_data, interval = intv)
...
#| fig-height: 5
# Filter for 1-minute interval
one_min_results <- aggregate_tfi(trade_data, quote_data, interval = "1 sec") 
...
#| fig-height: 5
# Filter for 1-minute interval
one_min_results <- aggregate_tfi(trade_data, quote_data, interval = "1 hour") 
...
ofi_data <- aggregate_ofi(quote_data, interval = "10 secs", tick_size = 0.1)
library(scales)
...
# Time Series TFI
```{r}
tfi_data <- aggregate_tfi(trade_data, quote_data, interval = "10 secs", tick_size = 0.1)
