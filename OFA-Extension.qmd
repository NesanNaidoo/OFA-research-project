...
```{r}
# ==========================================
# Stationarity Tests: ADF and KPSS (urca)
# ==========================================
...
# 2. KPSS test
# The null hypothesis (H0): series is stationary
kpss_test <- ur.kpss(update_counts, type = "tau")
summary(kpss_test)
...
kpss_stat <- kpss_test@teststat     # KPSS statistic
kpss_crit <- kpss_test@cval         # Critical values

stationarity_results <- data.frame(
  Test = c("ADF (trend)", "KPSS (tau)"),
  Statistic = c(round(adf_stat, 3), round(kpss_stat, 3)),
  `5% Critical Value` = c(adf_crit["5pct"], kpss_crit["5pct"]),
  `Decision (5%)` = c(
    ifelse(adf_stat < adf_crit["5pct"], "Reject H0 (Stationary)", "Fail to reject H0 (Non-stationary)"),
    ifelse(kpss_stat > kpss_crit["5pct"], "Reject H0 (Non-stationary)", "Fail to reject H0 (Stationary)")
  )
)
...
adf_urca <- ur.df(ofi_series_10s, type = "none", selectlags = "AIC")

# ADF test using ur.df with drift (constant) only
adf_test <- ur.df(ofi_series_10s, type = "none", selectlags = "AIC")
...
cat("Table 2: OFI Augmented Dickey–Fuller Test Results\n")
cat("-------------------------------------------------\n")
